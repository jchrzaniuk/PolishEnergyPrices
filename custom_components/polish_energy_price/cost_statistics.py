...
class ExternalCostStatisticsManager:
    ...
    async def _async_refresh(self) -> None:
        ...
        if set(mappings) != set(self.tariff.zones):
            _LOGGER.warning(
        ...
        for zone, source_id in mappings.items():
            rows = cumulative_cost_rows(source_rows.get(source_id, []), prices[zone])
            if not rows:
            ...
            async_add_external_statistics(
                self.hass,
                {
                    ...
                    "has_sum": True,
                    "name": (
                        f"{OPERATOR_NAMES[self.tariff.operator]} {self.tariff.group} "
                        f"— koszt, {ZONE_LABELS[zone]}"
                    ),
                    "source": DOMAIN,
